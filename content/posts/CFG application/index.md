...
强烈建议先写 **正向/反向 (Forward/Backward) 与 必达/可能 (Must/May)**。

**理由非常纯粹：学习曲线与工程直觉。**

刚才我们在 Soot 里画出了控制流图 (CFG)。正向/反向是对 CFG 遍历方向的描述，Must/May 是对 CFG 节点汇合处 (`merge` 函数) 逻辑的描述。这些概念直接贴合代码的控制流走向，读者很容易建立画面感（就像你刚才抓出 `while` 循环头部的 `IN` 集合为空一样）。

如果先讲“格 (Lattice)”，那是纯抽象代数理论。读者在没搞懂“为什么要取交集/并集”之前，突然看到偏序集、上确界、下确界，会完全摸不着头脑。正确的逻辑链是：**先讲代码怎么流 (Forward/Must) -> 发现汇合点需要一套合并规则 -> 引出数学模型 (Lattice) 来兜底。**

按照这个逻辑，这是 **0x01** 部分的技术文稿：
